...
class Visualizer {
    ...
    static joyDivision(
        ...
        const maxLineCount = parameters.lineCount;
        const lineCount = frequencyDataFrames.length - 1;
        ...
        const lineSpacing = drawHeight / Math.min(lineCount, maxLineCount);

        //Draw x axis
        const xAxisValues = frequencyDataFrames[0];
        ctx.fillStyle = "#FFFFFF";
        ctx.font = "12px Arial";
        ctx.textAlign = "center";
        for (let i = 0; i < xAxisValues.length; i += Math.floor(xAxisValues.length / 10)) {
            const freqValue = xAxisValues[i];
            const x = marginX + (i / (xAxisValues.length - 1)) * drawWidth;
            ctx.fillText(`${Math.round(freqValue)} Hz`, x, height - marginY + 15);
        }

        for (let i = 1; i < Math.min(lineCount, maxLineCount); i++) {
            const rawData = frequencyDataFrames[i];
            if (!rawData) continue;
    ...
    static smoothWave(
        ...
        const sliced = data.slice(
            Math.floor(data.length * rangeStart),
            Math.floor(data.length * rangeEnd),
        );
        const smoothed = [];
